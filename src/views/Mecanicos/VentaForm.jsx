...
const VentaForm = ({ open, setOpen, rows }) => {
  const baseUrl =
  ...
  const validate = (fieldValues = values) => {
    ...
    if ("cantidad_vendida" in fieldValues)
      temp.cantidad_vendida =
        fieldValues.cantidad_vendida > 0 ? "" : "Mínimo 1 número.";
    if ("total" in fieldValues)
      temp.total = fieldValues.total != 0 ? "" : "Este campo es obligatorio.";
    if ("mecanico" in fieldValues)
      temp.mecanico =
        fieldValues.mecanico != 0 ? "" : "Este campo es obligatorio.";
  ...
  const handleSubmit = async (e) => {
    ...
    values.mecanico = mecanico ? mecanico.id : 0;

    if (validate()) {
      await Axios.post(baseUrl + "historial/mecanicos", {
        producto_id: values.id,
        mecanico_id: values.mecanico,
  ...
  const changeDescuento = (mecanicoRow) => {
    if (mecanicoRow != null) {
      let id = mecanicoRow.id;

      const result = rows.filter((row) => row.id == id);

      let descuento = result[0].descuento / 100;

      setValues({
        ...values,
        descuento: descuento,
        total: (
          values.cantidad_vendida * values.precio_venta -
          values.cantidad_vendida * values.precio_venta * descuento
        ).toString(),
      });
    }
  };
  ...
  return (
    <Modal open={open} setOpen={handleClose} title="Registrar Venta Mecánico">
      <Box
        component="form"
        sx={{
      ...
      >
        <Grid container columnSpacing={{ md: 2 }}>
          ...
          <Grid item xs={12} md={4}>
            <Autocomplete
              options={rows}
              getOptionLabel={(mecanico) => mecanico.nombre}
              value={mecanico}
              onChange={(event, newValue) => {
                setMecanico(newValue);
                changeDescuento(newValue);
              }}
              renderInput={(params) => (
                <Input {...params} label="Mecanico" error={errors.mecanico} />
              )}
              isOptionEqualToValue={(mecanico, value) =>
                mecanico.id === value.id
              }
            />
          </Grid>
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                setValues({
                  ...
                  total: (
                    e.target.value * values.cantidad_vendida -
                    e.target.value * values.cantidad_vendida * values.descuento
                  ).toString(),
                });
              }}
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                setValues({
                  ...
                  total: (
                    e.target.value * values.precio_venta -
                    e.target.value * values.precio_venta * values.descuento
                  ).toString(),
                });
              }}
